function [ssr, msr, msrcr] = retinex(I)
% close all;
% clc; 
% I = imread('1.jpg');
Ir=I(:,:,1); 
...
%%%%%%%%%%가우시안 파라미터의 설정%%%%%% 
sigma_1=15;   %세 가우시안 커널
sigma_2=80; 
sigma_3=250; 
[x y]=meshgrid((-(size(Ir,2)-1)/2):(size(Ir,2)/2),(-(size(Ir,1)-1)/2):(size(Ir,1)/2));   
gauss_1=exp(-(x.^2+y.^2)/(2*sigma_1*sigma_1));  %계산 가우스 함수
Gauss_1=gauss_1/sum(gauss_1(:));  %정규화 
...
%%%%%%%%%%R 성분이 작동%%%%%%% 
% MSR 섹션 
Ir_log=log(Ir_double+1);  %는 대수 도메인으로 이미지를 변환 
f_Ir=fft2(Ir_double);  %화상 푸리에 주파수 영역으로 변환 

%Sigam = 15 결과 
fgauss=fft2(Gauss_1,size(Ir,1),size(Ir,2)); 
fgauss=fftshift(fgauss);  %주파수 영역의 중심이 제로로 이동
Rr=ifft2(fgauss.*f_Ir);  %는 회선은 공간 영역으로 다시 변환 할 
min1=min(min(Rr)); 
...
Rr=0.33*Rr1+0.34*Rr2+0.33*Rr3;   %가중 합
MSR1 = Rr;
SSR1 = Rr2;
%CR 계산  
...
%%%%%%%%%%G 구성 요소%%%%%%% 
Ig_double=double(Ig); 
Ig_log=log(Ig_double+1);  %는 대수 도메인으로 이미지를 변환 
f_Ig=fft2(Ig_double);  %화상 푸리에 주파수 영역으로 변환

fgauss=fft2(Gauss_1,size(Ig,1),size(Ig,2)); 
fgauss=fftshift(fgauss);  %주파수 영역의 중심이 제로로 이동 
Rg= ifft2(fgauss.*f_Ig);  %는 회선은 공간 영역으로 다시 변환 할 
min2=min(min(Rg)); 
Rg_log= log(Rg-min2+1); 
Rg1=Ig_log-Rg_log;  %의 sigam은 = 15 결과
...
Rg=0.33*Rg1+0.34*Rg2+0.33*Rg3;   %가중 합
SSR2 = Rg2;
MSR2 = Rg;
% CR 계산 
...
msrcr=cat(3,Rr_final,Rg_final,Rb_final);  % % 3 채널 화상 머징 

% subplot(2,2,1);imshow(I);title('포토 ')  %원 화상 디스플레이 
% subplot(2,2,2);imshow(ssr);title('SSR')
% subplot(2,2,3);imshow(msr);title('MSR')
% subplot(2,2,4);imshow(msrcr);title('MSRCR')  %처리 된 이미지를 표시 